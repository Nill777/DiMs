...
import com.distributedMessenger.data.network.webRTC.PeerConnectionManager
...
//    private val handshakeConnections = ConcurrentHashMap<UUID, PeerConnectionManager>()
    ...
    override fun sendHandshakeMessage(inviteId: UUID, message: DataMessage) {
        ...
        if (connection == null) {
            Logger.log(
                tag,
                "sendHandshakeMessage: No handshake connection found for '$inviteId'. Message not sent.",
                LogLevel.WARN
            )
            return
        }
        ...
        Logger.log(
            tag,
            "sendHandshakeMessage: Sending '${message::class.simpleName}' over handshake channel for '$inviteId'"
        )
    ...
    private suspend fun manageHandshake(inviteId: UUID, isInitiator: Boolean) {
        withContext(Dispatchers.Default) {
            ...
            if (handshakeConnections.containsKey(inviteId)) {
                Logger.log(
                    tag,
                    "manageHandshake: Already managing handshake for '$inviteId'.",
                    LogLevel.WARN
                )
                return@withContext
            }
            Logger.log(
                tag,
                "manageHandshake: Starting for room '$inviteId'. Is initiator: $isInitiator"
            )
            ...
            pcManager.dataChannelOpen
                .onEach { isOpen ->
                    if (isOpen) {
                        Logger.log(
                            tag,
                            "manageHandshake dataChannelOpen: Handshake DataChannel for '$inviteId' is OPEN. Notifying listeners."
                        )
            ...
            pcManager.outgoingSignal
                .onEach { signal ->
                    ...
                    Logger.log(
                        tag,
                        "Handshake: Forwarding outgoing '$signalType' to signaling client for room '$inviteId'"
                    )
            ...
            pcManager.incomingData
                .onEach { dataJson ->
                    try {
                        ...
                        Logger.log(
                            tag,
                            "Handshake: Received P2P data message '${dataMessage::class.simpleName}'"
                        )
                        _incomingMessages.emit(Pair("handshake_peer", dataMessage))
                    } catch (e: Exception) {
                        Logger.log(
                            tag,
                            "Handshake: Failed to parse incoming data: $dataJson",
                            LogLevel.ERROR,
                            e
                        )
                    }
            ...
            signalingClient.joinRoom(inviteId, myId)
                .onEach { peersMap ->
                    ...
                    if (otherPeer?.value != null) {
                        ...
                        val lastProcessedHash =
                            processedSignals.getOrPut(inviteId) { ConcurrentHashMap() }[peerId]
                        ...
                        if (currentSignalHash != lastProcessedHash) {
                            ...
                            try {
                                ...
                                Logger.log(
                                    tag,
                                    "Handshake: Could not parse signal from peer '$peerId'. Data: $signalData",
                                    LogLevel.WARN,
                                    e
                                )
                            }
