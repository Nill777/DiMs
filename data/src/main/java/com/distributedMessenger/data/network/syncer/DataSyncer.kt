package com.distributedMessenger.data.network.syncer
...
class DataSyncer(
    ...
    private fun handleChatMessage(msg: DataMessage.ChatMessage) {
        scope.launch {
            Logger.log(
                tag,
                "handleChatMessage Handling ChatMessage with id '${msg.messageId}'",
                LogLevel.DEBUG
            )
            ...
            if (messageRepository.getMessage(msg.messageId) == null) {
                ...
                Logger.log(
                    tag,
                    "handleChatMessage Message '${msg.messageId}' already exists. Ignoring.",
                    LogLevel.DEBUG
                )
            }
        }
    ...
    private fun handleSyncRequest(requesterId: PeerId, request: DataMessage.SyncRequest) {
        scope.launch {
            Logger.log(
                tag,
                "handleSyncRequest Handling SyncRequest from '$requesterId' for chat '${request.chatId}'"
            )
            ...
            if (messagesToSend.isNotEmpty()) {
                Logger.log(
                    tag,
                    "Found ${messagesToSend.size} messages to send back to '$requesterId'"
                )
                ...
                Logger.log(
                    tag,
                    "handleSyncRequest No new messages found for sync request from '$requesterId'",
                    LogLevel.DEBUG
                )
            }
        }
    ...
    private fun handleSyncResponse(response: DataMessage.SyncResponse) {
        scope.launch {
            Logger.log(
                tag,
                "handleSyncResponse Handling SyncResponse with ${response.messages.size} messages."
            )
