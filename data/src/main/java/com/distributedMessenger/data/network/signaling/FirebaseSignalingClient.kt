...
//            }
        ...
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                Logger.log(
                    tag,
                    "onDataChange triggered for room '$chatId'. Found ${snapshot.childrenCount} peers.",
                    LogLevel.DEBUG
                )
                ...
                Logger.log(
                    tag,
                    "onDataChange Room state updated. Peers found: ${peersMap.keys}",
                    LogLevel.DEBUG
                )
            ...
            override fun onCancelled(error: DatabaseError) {
                Logger.log(
                    tag,
                    "onCancelled Listener for room '$chatId' was cancelled. Error: ${error.message}",
                    LogLevel.ERROR,
                    error.toException()
                )
        ...
        // Говорит серверу Firebase: Если я вдруг отключусь нештатно (пропадет интернет, закроется приложение),
        // тогда выполни эту инструкцию и удали мой узел
        awaitClose {
...
//        myRef.setValue(signalJson)
        ...
        myRef.setValue(signalJson)
            .addOnSuccessListener {
                Logger.log(
                    tag,
                    "sendSignal: Successfully wrote to path: ${myRef.toString()}",
                    LogLevel.DEBUG
                )
            }
            .addOnFailureListener { e ->
                Logger.log(
                    tag,
                    "sendSignal: FAILED to write to path: ${myRef.toString()}",
                    LogLevel.ERROR,
                    e
                )
            }
