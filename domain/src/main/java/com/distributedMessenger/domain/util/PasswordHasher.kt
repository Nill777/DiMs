package com.distributedMessenger.domain.util
...
//        "\$argon2id\$i=$ITERATIONS\$m=$MEMORY_COST_KB\$p=$PARALLELISM$$saltB64$$hashB64"
    ...
    fun verifyPassword(password: String, storedHash: String, pepper: String): Boolean {
        return try {
            ...
            Logger.log("PasswordHasher", "Invalid hash format (bad Base64)", LogLevel.ERROR, e)
            false
        }
//        } catch (e: IllegalArgumentException) {
//            Logger.log("PasswordHasher", "Invalid hash format (bad Base64)", LogLevel.ERROR, e)
...
//            Logger.log("PasswordHasher", "Invalid hash format (could not parse numbers)", LogLevel.ERROR, e)
//            false
//        } catch (e: IndexOutOfBoundsException) {
//            Logger.log("PasswordHasher", "Invalid hash format (incorrect number of parts)", LogLevel.ERROR, e)
//            false
//        }
    }
