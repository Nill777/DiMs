...
class MessageServiceUnitTest {
    ...
    fun setup() {
        ...
        messageService =
            MessageService(mockMessageRepository, mockHistoryRepository, mockP2pTransport)
    }
    ...
    fun `sendMessage should save message, send it to network and return id`() = runTest {
        ...
        assertEquals(
            expectedMessageId,
            result,
            "Метод должен возвращать ID, полученный от репозитория"
        )
    ...
    fun `getMessage should return message when it exists`() = runTest {
        ...
        val expectedMessage = TestObjectMother.createMessage(
            id = messageId,
    ...
    @Test
    fun `editMessage should update message, add history record and return true on success`() =
        runTest {
            // Arrange
            val messageId = UUID.randomUUID()
            val originalContent = "Original Content"
            val newContent = "New Content"
            val originalMessage = TestObjectMother.createMessage(
                id = messageId,
                chatId = UUID.randomUUID(),
                senderId = UUID.randomUUID()
            )
            ...
            coVerify(exactly = 1) {
                mockHistoryRepository.addMessageHistory(match {
