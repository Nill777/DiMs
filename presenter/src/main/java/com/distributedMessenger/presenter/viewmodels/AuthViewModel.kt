...
class AuthViewModel(private val userService: IUserService) : ViewModel() {
    ...
    private suspend fun handleLoginResult(result: LoginResult, username: String) {
        when (result) {
            is LoginResult.Success -> {
                ...
                SessionManager.login(
                    userId,
                    username,
                    userService.getUser(userId)?.role ?: UserRole.UNAUTHORIZED_USER
                )
            ...
            is LoginResult.WrongPassword -> {
                _authState.value =
                    AuthState.Error("Invalid password. Attempts remaining: ${result.remainingAttempts}")
            }
